...
import { LESSONS, TAGS, T, CURRENT_STREAK_LABEL, BOOKS } from '../data.js';
...
export default function HomeScreen({ onOpenBook }) {
  ...
  const filters = [
    ...
    ...Array.from(new Set(LESSONS.map(l => l.tag))).slice(0, 6).map(t => ({ id: `tag:${t}`, label: TAGS[t].label })),
  ];
    ...
    <div className="screen-in">
      <div className="pad-page" style={{ padding: '36px 40px 28px', borderBottom: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'flex', gap: 24, marginTop: 22, color: 'var(--fg-muted)', fontSize: 13, flexWrap: 'wrap' }}>
          ...
          <div><span style={{ color: 'var(--fg)', fontWeight: 500 }}>{CURRENT_STREAK_LABEL}</span></div>
        </div>
      </div>
