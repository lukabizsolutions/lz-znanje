// Tweaks panel — color, font, density, cover style, sidebar width
...
const DENSITY_OPTS = [
  ...
  { id: 'roomy', label: 'Roomy', scale: 1.14 },
];
      ...
      <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 18, maxHeight: "70vh", overflowY: 'auto' }}>
        ...
        <Field label="Accent">
          <div style={{ display: 'flex', gap: 6 }}>
            {ACCENT_HUES.map(a => (
              <button key={a.h} title={a.name} onClick={() => update({ accentHue: a.h })}
                style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {Object.entries(FONT_PAIRS).map(([id, p]) => (
              <button key={id} onClick={() => update({ fontPair: id })} style={{
                ...
                {tweaks.fontPair === id && <window.IconCheck size={12} />}
              </button>
            ))}
          </div>
        </Field>
