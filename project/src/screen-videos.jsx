// Video lekcije — Netflix-style sa play, trajanjem, opisom
    ...
    <button onClick={onOpen} style={{
      ...
      onMouseLeave={e => { const p = e.currentTarget.querySelector(".play"); if (p) p.style.transform = "scale(1)"; }}>
        ...
        <div className="mono" style={{
          ...
          fontSize: 10.5, color: 'white', borderRadius: 4,
      ...
      {!large && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11.5, color: "var(--fg-muted)" }}>
          <window.TagPill tag={video.tag} size="xs" />
          <span>·</span>
          <span>{video.views} {window.T.common.views}</span>
        </div>
      )}
    </button>
  );
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding: "26px 30px 30px" }}>
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 14, flexWrap: 'wrap' }}>
            <window.TagPill tag={video.tag} />
            <div className="micro">{window.T.common.recorded} · {video.date}</div>
            <div className="micro">{video.views} {window.T.common.views}</div>
          </div>
  ...
  return (
    ...
    onMouseLeave={e => { e.currentTarget.style.borderColor = "var(--border-strong)"; e.currentTarget.style.color = "var(--fg-muted)"; }}>
      <div style={{
        ...
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M12 16V5"/><path d="m7 10 5-5 5 5"/><path d="M5 19h14"/></svg>
      </div>
      <div className="serif" style={{ fontSize: 14, fontWeight: 500 }}>{window.T.common.uploadNew}</div>
    </button>
  );
}
...
function VideosScreen() {
  ...
  const featured = window.VIDEOS[0];
  const rest = window.VIDEOS.slice(1);
    ...
    <div className="screen-in">
      <div className="pad-page" style={{ padding: "36px 40px 28px", borderBottom: "1px solid var(--border)" }}>
        <div style={{ display: 'flex', justifyContent: "space-between", alignItems: "flex-end", gap: 20, flexWrap: 'wrap' }}>
          <div>
            <div className="micro" style={{ marginBottom: 10 }}>{window.T.nav.videos} · {window.VIDEOS.length}</div>
          ...
          <window.PrimaryButton icon={<window.IconPlus size={13} stroke={2} />}>{T.uploadNew}</window.PrimaryButton>
        </div>
      </div>
      ...
      <div className="pad-page" style={{ padding: "32px 40px 80px" }}>
        ...
        <section style={{ marginBottom: 40 }}>
          ...
          <div className="two-col" style={{
            ...
            <div>
              <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 14, flexWrap: 'wrap' }}>
                <window.TagPill tag={featured.tag} />
              ...
              <p style={{
                margin: "0 0 18px", fontSize: 14.5, color: "var(--fg-muted)",
                lineHeight: 1.6,
              }}>{featured.description}</p>
              <window.GhostButton onClick={() => setOpen(featured)}
