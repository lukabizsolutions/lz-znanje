    ...
    <button onClick={onOpen} style={{
      ...
      <div style={{ width: "100%", aspectRatio: "2 / 3", position: 'relative' }}>
        <div style={{ position: 'absolute', inset: 0 }}>
          <window.BookCover book={book} w="100%" h="100%" style={{ width: "100%", height: "100%" }} />
        </div>
        {!book.finished && (
          <div style={{
      ...
      <div>
        ...
        <div className="micro" style={{ marginTop: 4 }}>
          {book.finished ? `${T.read} · ${book.year}` : T.reading}
        </div>
      </div>
    </button>
  );
...
function LibraryScreen({ onOpenBook }) {
  const [query, setQuery] = React.useState("");
  const [sort, setSort] = React.useState("recent");
  const T = window.T.common;

  const books = React.useMemo(() => {
    let list = window.BOOKS;
    if (query.trim()) {
    ...
    <div className="screen-in">
      <div className="pad-page" style={{ padding: "36px 40px 24px", borderBottom: "1px solid var(--border)" }}>
        <div style={{ display: 'flex', alignItems: "flex-end", justifyContent: "space-between", gap: 24, flexWrap: 'wrap' }}>
          <div>
            <div className="micro" style={{ marginBottom: 10 }}>{window.T.nav.library} · {window.BOOKS.length} knjiga</div>
            <h1 className="serif hero-title" style={{
              margin: 0, fontSize: 38, fontWeight: 500, letterSpacing: "-0.022em",
            }}>{T.theShelf}</h1>
          </div>
          <window.PrimaryButton icon={<window.IconPlus size={13} stroke={2} />}>{T.addBook}</window.PrimaryButton>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 26, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {[{ id: 'recent', label: T.recent }, { id: 'title', label: T.title }, { id: 'author', label: T.author }].map(s => (
      ...
      <div className="pad-page" style={{ padding: "32px 40px 80px" }}>
        {reading.length > 0 && (
          <section style={{ marginBottom: 44 }}>
            <div className="micro" style={{ marginBottom: 16 }}>{T.currentlyReading}</div>
            <div style={{
              display: 'grid',
        ...
        <section>
          <div className="micro" style={{ marginBottom: 16 }}>{T.read} · {finished.length}</div>
          <div style={{
            display: 'grid',
            gridTemplateColumns: "repeat(auto-fill, minmax(130px, 1fr))",
            gap: "32px 16px",
